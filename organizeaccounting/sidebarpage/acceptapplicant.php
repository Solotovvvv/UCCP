  ...
  if(isset($_POST['acceptapp'])){
    ...
    '', '', '', '', '', '', '' FROM `uccp_admission_billing` WHERE id = '$id';";
     mysqli_query($conn,$query);
       ...
       $query1 = "INSERT INTO `uccp_accpaidaddmission`(`id`, `name`,`ornumber`, `status`) SELECT `id`, `name`, `ornumber`, `status` FROM `uccp_admission_billing` WHERE id = '$id';";
        mysqli_query($conn,$query1);

        $query2 = "UPDATE `uccp_accpaidaddmission` SET status = 'Paid'  WHERE id = '$id';";
